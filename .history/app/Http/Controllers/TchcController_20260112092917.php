...
class TchcController extends Controller
{
    ...
    public function checkerDashboard()
    {
        ...
        // Build query - Lấy danh sách phiếu đã được duyệt bởi approver, chờ TCHC check
        $query = SupplyRequest::where('status', 'approved')
        ...
        $pendingRequests = $query->orderBy('approved_at', 'asc')->paginate(10);

        // Lấy danh sách bộ phận để filter
        $departments = SupplyRequest::where('status', 'approved')
            ->distinct()
            ->pluck('requester_department')
            ->filter()
            ->sort();

        return view('tchc.checker.dashboard', compact('pendingRequests', 'departments'));
    }
