...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        // TCHC Manager chỉ có quyền phê duyệt VPP
        if ($user->isTchcManager()) {
            return redirect()->route('tchc.manager.dashboard');
        }
        ...
        // Employee chỉ có chức năng đăng ký VPP
        if ($user->isEmployee()) {
    ...
    public function employee()
    {
        ...
        $myRequests = SupplyRequest::where('user_id', $user->id)
            ->with([
                'requestItems:id,request_id,office_supply_id,quantity',
                'requestItems.officeSupply:id,name',
        ...
        $stats = [
            ...
            'my_approved' => SupplyRequest::where('user_id', $user->id)->where('status', 'approved')->count(),
            'my_rejected' => SupplyRequest::where('user_id', $user->id)->where('status', 'rejected')->count(),
    ...
    public function approval()
    {
        ...
        // Đơn chờ phê duyệt (bao gồm pending và forwarded)
        ...
        // Thống kê
        $stats = [
            'pending_count' => (clone $requestsQuery)->whereIn('status', ['pending', 'forwarded'])->count(),
    ...
    public function approve(Request $request, SupplyRequest $supplyRequest)
    {
        ...
        $message = $request->action === 'approve' ? 'Đơn yêu cầu đã được phê duyệt.' : 'Đơn yêu cầu đã bị từ chối.';
