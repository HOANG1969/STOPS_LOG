        ...
        <form action="{{ route('employee.requests.store') }}" method="POST" id="requestForm">
            ...
            <div class="flex justify-between">
                ...
                <div class="flex gap-2">
                    <button type="button" onclick="debugFormData()" class="bg-yellow-500 hover:bg-yellow-600 text-white px-4 py-2 rounded-md text-sm">
                        Debug
                    </button>
...
function debugFormData() {
    ...
    console.log('Table rows:', document.querySelectorAll('#supplyTableBody tr:not(#emptyTableMessage)').length);
    
    const inputs = document.querySelectorAll('input[name*="items["]');
    console.log('Items inputs found:', inputs.length);
...
document.getElementById('requestForm').addEventListener('submit', function(e) {
    ...
    // Check if items exist in form data
    let hasItems = false;
    for (let [key, value] of formData.entries()) {
    ...
    if (!hasItems) {
        e.preventDefault();
        alert('Lỗi: Không tìm thấy dữ liệu items trong form. Vui lòng thử lại!');
        return;
    }
    
    console.log('Form data validation passed, submitting...');
});
</script>
@endsection