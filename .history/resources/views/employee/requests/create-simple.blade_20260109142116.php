    ...
    if (e.target.classList.contains('select-supply')) {
        ...
        newRow.innerHTML = `
            ...
            <td class="px-4 py-2 border">
                ${name}
                <input type="hidden" name="items[${rowIndex}][supply_id]" value="${id}" form="requestForm">
            </td>
            <td class="px-4 py-2 border">${unit}</td>
            <td class="px-4 py-2 border">
                <input type="number" name="items[${rowIndex}][quantity]" 
                       class="w-full px-2 py-1 border rounded" 
                       min="1" max="${stock}" required form="requestForm">
            </td>
            <td class="px-4 py-2 border">
                <input type="text" name="items[${rowIndex}][purpose]" 
                       class="w-full px-2 py-1 border rounded" 
                       placeholder="Ghi chú (tùy chọn)" form="requestForm">
            </td>
            <td class="px-4 py-2 border">
        ...
        console.log('Current inputs:', document.querySelectorAll('input[name*="items["]'));
...
document.getElementById('requestForm').addEventListener('submit', function(e) {
    ...
    // Kiểm tra các quantity input trong form
    ...
    // Kiểm tra xem có input items trong form không
    const itemInputs = form.querySelectorAll('input[name*="items["]');
    if (itemInputs.length === 0) {
        e.preventDefault();
        alert('Lỗi: Không tìm thấy dữ liệu items. Vui lòng thử thêm lại văn phòng phẩm!');
        return;
    }
});
</script>
