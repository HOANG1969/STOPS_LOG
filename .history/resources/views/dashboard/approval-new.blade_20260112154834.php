...
function loadApprovalRequests() {
    ...
    $.ajax({
        url: '{{ route("supply-requests.for-approval") }}',
        method: 'GET',
        success: function(requests) {
            console.log('Loaded requests:', requests);
            allRequestsData = requests; // Store globally
...
function loadAllRequests(filterType = null) {
    ...
    $.ajax({
        url: '{{ route("supply-requests.index") }}',
        method: 'GET',
        data: { 
            ajax: true,
            user_requests: filterType === 'user' ? 1 : 0
        },
        success: function(response) {
            console.log('Loaded all requests:', response);
            // Handle both direct array and object with data property
            const allRequests = response.data ? response.data : response;
...
function renderTable(requests) {
        ...
        tbody.append(`
            <tr>
                <td colspan="8" class="text-center text-muted py-4">
                    ...
                    <div>Không có yêu cầu nào chờ phê duyệt</div>
                </td>
            </tr>
        `);
        return;
    ...
    requests.forEach(function(request) {
        ...
        const totalValue = calculateTotalValue(request.request_items);
        const itemsPreview = getItemsPreview(request.request_items);
...
function calculateTotalValue(items) {
    return items.reduce((total, item) => {
        return total + (parseFloat(item.office_supply.price) * parseInt(item.quantity));
    }, 0);
}

function getItemsPreview(items) {
    const preview = items.slice(0, 2).map(item => 
        `${item.office_supply.name} (${item.quantity})`
    ).join(', ');
...
function filterByStatus(status) {
    ...
    switch(status) {
        ...
        case 'approved_month':
            ...
            break;
        case 'all':
            // For 'all', we need to fetch all requests, not just pending ones
            loadAllRequests(status);
            return;
        default:
