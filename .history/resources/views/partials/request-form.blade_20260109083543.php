...
function loadSuppliesIntoModal() {
    ...
    tbody.html('<tr><td colspan="7" class="text-center"><i class="fas fa-spinner fa-spin me-2"></i>Đang tải...</td></tr>');
        ...
        const row = `
            <tr data-supply-id="${supply.id}">
                ...
                <td>
                    ...
                    <small class="text-muted">Giá: ${new Intl.NumberFormat('vi-VN', {style: 'currency', currency: 'VND'}).format(supply.price)}</small>
                </td>
                <td><small>${supply.description || 'Không có mô tả'}</small></td>
                ...
                <td class="text-center">
                    <button type="button" 
                            class="btn ${isAlreadySelected ? 'btn-secondary' : 'btn-primary'} btn-sm" 
...
function loadOfficeSuppliesForRequest(callback = null) {
    $.ajax({
        ...
        success: function(supplies) {
            ...
            console.log('Loaded supplies:', supplies.length); // Debug log
