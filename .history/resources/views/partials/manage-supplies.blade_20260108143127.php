...
function editSupply(supplyId) {
    ...
    $.get(`{{ route('office-supplies.admin') }}`, function(supplies) {
...
function saveSupply() {
    ...
    const url = isEdit ? `{{ url('office-supplies') }}/${supplyId}` : `{{ route('office-supplies.store') }}`;
...
function deleteSupply(supplyId) {
    ...
    $.ajax({
        url: `{{ url('office-supplies') }}/${supplyId}`,
        method: 'DELETE',
        data: {
