...
if ($_SERVER['REQUEST_URI'] === '/register') {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $newId = 'VP' . sprintf('%03d', count($_SESSION['requests']) + 1);
        ...
        $_SESSION['message'] = 'Đăng ký văn phòng phẩm thành công! Mã đơn: ' . $newId;
        $_SESSION['new_request_id'] = $newId;
        ...
        <div class="container-fluid">
            <div class="row">
                ...
                <div class="col-md-3 sidebar text-white p-0">
                    <div class="p-3">
                        <h4><i class="fas fa-building"></i> Office Supplies</h4>
                        <p class="mb-0">Xin chào: <strong><?= $_SESSION['user_name'] ?? 'User' ?></strong></p>
                        <hr>
                        <nav class="nav flex-column">
...
if ($_SERVER['REQUEST_URI'] === '/requests') {
    ...
    // Handle forwarding to leader
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['forward_to_leader'])) {
        $requestId = $_POST['request_id'];
        if (isset($_SESSION['requests'][$requestId])) {
            $_SESSION['requests'][$requestId]['status'] = 'forwarded';
            $_SESSION['requests'][$requestId]['approval_history'][] = [
                'action' => 'forwarded',
                'by' => $_SESSION['user_name'],
                'date' => date('Y-m-d H:i:s'),
                'note' => 'Chuyển đến lãnh đạo phê duyệt'
            ];
            $_SESSION['message'] = 'Đã chuyển đơn ' . $requestId . ' đến lãnh đạo phê duyệt!';
        }
        header('Location: /requests');
        exit;
    }
    ...
    $search = $_GET['search'] ?? '';
    $filteredRequests = $_SESSION['requests'];
    
    // Apply search filter
    if (!empty($search)) {
        $filteredRequests = array_filter($_SESSION['requests'], function($request) use ($search) {
            return stripos($request['id'], $search) !== false ||
                   stripos($request['requester'], $search) !== false ||
    ...
    <head>
        ...
        <style>
            ...
            .content { min-height: 100vh; background: #f8f9fa; }
            .status-badge { font-size: 0.8em; }
            .new-request { border-left: 4px solid #28a745; background-color: #f8fff9; }
        </style>
    </head>
    <body>
        <div class="container-fluid">
                            ...
                            <form method="GET" class="row">
                                <div class="col-md-6">
                                    <div class="input-group">
                                        ...
                                        <input type="text" name="search" class="form-control" placeholder="Tìm kiếm theo mã đơn, tên, phòng ban..." value="<?= htmlspecialchars($search) ?>">
                                ...
                                <div class="col-md-3">
                                    <select class="form-select" name="status">
                                        ...
                                        <option value="forwarded">Đã chuyển lãnh đạo</option>
                        ...
                        <div class="card-body">
                            <?php if (empty($filteredRequests)): ?>
                                <div class="text-center py-4">
                                    ...
                                    <p class="text-muted">Không tìm thấy kết quả nào cho "<?= htmlspecialchars($search) ?>"</p>
                                    <a href="/requests" class="btn btn-outline-primary">Xem tất cả</a>
                                </div>
                            <?php else: ?>
                            <div class="table-responsive">
                                <table class="table">
                                    ...
                                    <tbody>
                                        <?php foreach ($filteredRequests as $request): ?>
                                            <?php
                                            $isNewRequest = isset($_SESSION['new_request_id']) && $_SESSION['new_request_id'] === $request['id'];
                                            if ($isNewRequest) unset($_SESSION['new_request_id']);
                                            
                                            // Format items for display
                                            $itemsText = '';
                                            foreach ($request['items'] as $i => $item) {
                                                if ($i > 0) $itemsText .= ', ';
                                                $itemsText .= $item['name'] . ' (' . $item['quantity'] . ' ' . $item['unit'] . ')';
                                            }
                                            if (strlen($itemsText) > 50) {
                                                $itemsText = substr($itemsText, 0, 50) . '...';
                                            }
                                            ?>
                                            <tr class="<?= $isNewRequest ? 'new-request' : '' ?>">
                                                <td>
                                                    <strong><?= htmlspecialchars($request['id']) ?></strong>
                                                    <?php if ($isNewRequest): ?>
                                                        <span class="badge bg-success ms-1">Mới</span>
                                                    <?php endif; ?>
                                                </td>
                                                <td><?= htmlspecialchars($request['requester']) ?></td>
                                                <td><?= htmlspecialchars($request['department']) ?></td>
                                                <td><?= htmlspecialchars($itemsText) ?></td>
                                                <td><?= date('d/m/Y', strtotime($request['created_at'])) ?></td>
                                                <td><?= date('d/m/Y', strtotime($request['needed_date'])) ?></td>
                                                <td>
                                                    <?php 
                                                    $statusClass = [
                                                        'pending' => 'bg-warning',
                                                ...
                                                <td>
                                                    <a href="/detail/<?= $request['id'] ?>" class="btn btn-sm btn-outline-primary">Chi tiết</a>
                                                    
                                                    <?php if ($request['status'] === 'pending' && ($_SESSION['user_role'] ?? '') === 'employee'): ?>
                                                        <form method="POST" style="display: inline;">
                                                            <input type="hidden" name="request_id" value="<?= $request['id'] ?>">
                                                            <button type="submit" name="forward_to_leader" class="btn btn-sm btn-warning ms-1" 
                                                                    onclick="return confirm('Chuyển đơn này đến lãnh đạo phê duyệt?')">
                                                                <i class="fas fa-arrow-right"></i> Chuyển lãnh đạo
                                                            </button>
                                                        </form>
                                                    <?php endif; ?>
                                                </td>
...
if ($_SERVER['REQUEST_URI'] === '/approval') {
    if (!isset($_SESSION['logged_in']) || $_SESSION['user_role'] !== 'leader') {
        header('Location: /dashboard');
        exit;
    }
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['decision'])) {
        ...
        $note = $_POST['approval_note'] ?? '';
        
        if (isset($_SESSION['requests'][$requestId])) {
            $newStatus = $decision === 'approve' ? 'approved' : 'rejected';
            $_SESSION['requests'][$requestId]['status'] = $newStatus;
            $_SESSION['requests'][$requestId]['approval_history'][] = [
                'action' => $newStatus,
                'by' => $_SESSION['user_name'] . ' (Lãnh đạo)',
                'date' => date('Y-m-d H:i:s'),
                'note' => $note
            ];
            
            $actionText = $decision === 'approve' ? 'phê duyệt' : 'từ chối';
            $_SESSION['message'] = 'Đã ' . $actionText . ' đơn ' . $requestId . ' thành công!';
        }
        header('Location: /approval');
        exit;
    }
    
    // Get pending requests for leader
    $pendingRequests = array_filter($_SESSION['requests'], function($request) {
        return in_array($request['status'], ['forwarded', 'pending']);
    });
    ...
    <body>
        <div class="container-fluid">
            <div class="row">
                ...
                <div class="col-md-9 content p-4">
                    ...
                    <div class="row mb-4">
                        <?php
                        $totalPending = count(array_filter($_SESSION['requests'], fn($r) => in_array($r['status'], ['pending', 'forwarded'])));
                        $totalApproved = count(array_filter($_SESSION['requests'], fn($r) => $r['status'] === 'approved'));
                        $totalRejected = count(array_filter($_SESSION['requests'], fn($r) => $r['status'] === 'rejected'));
                        $highPriority = count(array_filter($_SESSION['requests'], fn($r) => $r['priority'] === 'High' && in_array($r['status'], ['pending', 'forwarded'])));
                        ?>
                        <div class="col-md-3">
                            <div class="card text-white bg-warning">
                                <div class="card-body">
                        ...
                        <div class="card-body">
                            <?php if (empty($pendingRequests)): ?>
                                <div class="text-center py-5">
                                    ...
                                    <h4>Không có đơn nào chờ phê duyệt</h4>
                            ...
                            <div class="list-group">
                                <?php foreach ($pendingRequests as $request): ?>
                                    ...
                                    elseif ($request['priority'] === 'Urgent') $priorityClass = 'priority-urgent';
                                    ...
                                    <div class="list-group-item <?= $priorityClass ?>">
                                        <div class="d-flex w-100 justify-content-between">
                                            <div>
                                                <h5 class="mb-1"><?= $request['id'] ?> - <?= htmlspecialchars($request['requester']) ?> (<?= htmlspecialchars($request['department']) ?>)</h5>
                                                <p class="mb-1"><?= htmlspecialchars($itemsText) ?></p>
                                                <small>Đăng ký: <?= date('d/m/Y', strtotime($request['created_at'])) ?> | Cần có: <?= date('d/m/Y', strtotime($request['needed_date'])) ?></small>
                                                <?php if (!empty($request['notes'])): ?>
                                                    <br><small class="text-muted">Ghi chú: <?= htmlspecialchars($request['notes']) ?></small>
                                                <?php endif; ?>
                                            </div>
                                            <div class="text-end">
                                                <?php 
                                                ...
                                                ?>
                                                <span class="badge <?= $priorityBadgeClass ?> mb-2"><?= $request['priority'] === 'Normal' ? 'Bình thường' : ($request['priority'] === 'High' ? 'Ưu tiên cao' : 'Khẩn cấp') ?></span><br>
                                                <div class="btn-group" role="group">
                                                    <a href="/detail/<?= $request['id'] ?>" class="btn btn-sm btn-outline-primary">Chi tiết</a>
                                                    <button class="btn btn-sm btn-success" onclick="showApprovalModal('<?= $request['id'] ?>', 'approve')">Duyệt</button>
                                                    <button class="btn btn-sm btn-danger" onclick="showApprovalModal('<?= $request['id'] ?>', 'reject')">Từ chối</button>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                ...
                <div class="modal-content">
                    ...
                    <form method="POST">
                        <div class="modal-body">
                            ...
                            <div class="mb-3">
                                <label class="form-label">Ghi chú phê duyệt</label>
                                <textarea class="form-control" name="approval_note" rows="3" placeholder="Nhập ghi chú..."></textarea>
                            </div>
                        </div>
        ...
        <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js"></script>
        <script>
            function showApprovalModal(requestId, decision) {
                ...
                if (decision === 'approve') {
                    modalTitle.textContent = 'Phê duyệt đơn ' + requestId;
                    modalAlert.className = 'alert alert-success';
                    modalAlert.textContent = 'Bạn có chắc chắn muốn phê duyệt đơn này?';
                    modalSubmitBtn.className = 'btn btn-success';
                    modalSubmitBtn.textContent = 'Phê duyệt';
                } else {
                    modalTitle.textContent = 'Từ chối đơn ' + requestId;
                    modalAlert.className = 'alert alert-danger';
                    modalAlert.textContent = 'Bạn có chắc chắn muốn từ chối đơn này?';
                    modalSubmitBtn.className = 'btn btn-danger';
                    modalSubmitBtn.textContent = 'Từ chối';
                }
