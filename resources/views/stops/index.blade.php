...
<div class="container-fluid">
        ...
        <div class="card-body">
            <form method="GET" action="{{ route('stops.index') }}" class="row g-3">
                <div class="col-12 col-md-6 col-lg-3 col-xl-2">
                    ...
                    <input type="text" name="observer_name" class="form-control" value="{{ request('observer_name') }}" placeholder="Tên người ghi nhận">
                </div> 
                <div class="col-12 col-md-6 col-lg-3 col-xl-2">
                ...
                <div class="col-12 col-md-6 col-lg-3 col-xl-2">
                    <label class="form-label">Ca/kíp</label>
                        <select name="shift" class="form-select">
                            ...
                            <option value="HTSX" {{ request('shift') == 'HTSX' ? 'selected' : '' }}>HTSX</option>
                            <option value="VH01" {{ request('shift') == 'VH01' ? 'selected' : '' }}>VH01</option>
                            <option value="VH02" {{ request('shift') == 'VH02' ? 'selected' : '' }}>VH02</option>
                            <option value="VH03" {{ request('shift') == 'VH03' ? 'selected' : '' }}>VH03</option>
                            <option value="VH04" {{ request('shift') == 'VH04' ? 'selected' : '' }}>VH04</option>
                        </select>
                </div>  
                <div class="col-12 col-md-6 col-lg-3 col-xl-2">
                ...
                <div class="col-12 col-md-6 col-lg-3 col-xl-2">
                    ...
                    <select name="status" class="form-select">
                        <option value="">Tất cả</option>
                        <option value="open" {{ request('status') == 'open' ? 'selected' : '' }}>Chưa xử lý</option>
    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            @if(Auth::user()->isAdmin() || Auth::user()->isApprover() || Auth::user()->isTchcChecker() || Auth::user()->isTchcManager())
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td data-label="Chọn">
                                ...
                                <input type="checkbox" class="form-check-input stop-checkbox" value="{{ $stop->id }}" data-stop-id="{{ $stop->id }}">
                                @endif
                            </td>
                            @endif
                            <!-- <td>{{ $stops->firstItem() + $index }}</td> -->
                            <td data-label="Thời gian đăng ký">
                                {{ $stop->created_at->format('d/m/Y') }}
                                <br><small class="text-muted">{{ $stop->created_at->format('H:i') }}</small>
                            </td>
                            <td data-label="Người ghi nhận">{{ $stop->observer_name }}</td>
                            <!-- <td>{{explode('@',$stop->user->email)[0]}}</td> -->
                            <td data-label="Ca/kíp">{{ $stop->observer_phone }}</td>
                            <td data-label="Loại vấn đề">
                                <span class="badge bg-info">{{ $stop->getCategoryLabel() }}</span>
                            </td>
                            <td data-label="Vị trí">{{ Str::limit($stop->location,10,'...') }}</td>
                            <td data-label="Thiết bị">{{ Str::limit($stop->equipment_name ?? '-',10,'...') }}</td>
                            <td data-label="Nội dung">
                                <small>{{ Str::words($stop->issue_description, 10,'...') }}</small>
                            </td>
                            <td data-label="Đề xuất hành động">
                                <small>{{ Str::words($stop->corrective_action, 10, '...') }}</small>
                            </td>
                            <td class="text-center align-middle" data-label="Mức độ">
                                <div class="d-flex align-items-center justify-content-center gap-2">
                                    ...
                                    @if($canInlineScore && $stop->priority_level === null)
                                        <select class="form-select form-select-sm priority-inline-select"
                                                data-stop-id="{{ $stop->id }}"
                                            style="min-width: 110px; max-width: 120px;">
                                            <option value="" {{ $stop->priority_level === null ? 'selected' : '' }}>Chưa chấm</option>
                                            <option value="0" {{ (string)$stop->priority_level === '0' ? 'selected' : '' }}>Mức 0</option>
                                            <option value="1" {{ (string)$stop->priority_level === '1' ? 'selected' : '' }}>Mức 1</option>
                                            <option value="2" {{ (string)$stop->priority_level === '2' ? 'selected' : '' }}>Mức 2</option>
                                            <option value="3" {{ (string)$stop->priority_level === '3' ? 'selected' : '' }}>Mức 3</option>
                                        </select>
                                    @elseif($stop->priority_level !== null)
                                        <span class="badge {{ $stop->getPriorityBadgeClass() }}">
                            ...
                            <td data-label="Trạng thái">
                                <span class="{{ $stop->getStatusBadgeClass() }}">
                                    {{ $stop->getStatusLabel() }}
                                </span>
                            </td>
                            <td data-label="Thao tác">
                                <div class="btn-group" role="group">
                                    ...
                                    <a href="{{ route('stops.show', $stop) }}" class="btn btn-sm btn-info" title="Xem chi tiết">
                                        <i class="fas fa-eye"></i>
                                    </a>
                                    @endif

                                    @if($canEditStop)
                                        <a href="{{ route('stops.edit', $stop) }}" class="btn btn-sm btn-warning" title="Sửa">
                                    ...
                                    @if(Auth::user()->isAdmin() || Auth::user()->isApprover() || Auth::user()->isTCHCManager())
                                        @if($stop->status === 'completed')
                                            ...
                                            <a href="{{ route('stops.show', $stop) }}" class="btn btn-sm btn-secondary" title="Xem chi tiết (Đã hoàn thành)">
                                                <i class="fas fa-eye"></i>
                                            </a>
                                        @else
                                            <!-- STOP chưa hoàn thành -->
                                            <form action="{{ route('stops.destroy', $stop) }}" method="POST" class="d-inline" onsubmit="return confirm('Bạn có chắc muốn xóa?')">
                                                @csrf
                                                @method('DELETE')
