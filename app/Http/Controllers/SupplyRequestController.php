...
use App\Exports\SupplyItemsExport;
...
class SupplyRequestController extends Controller
{
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        try {
            ...
            $supplyRequest = SupplyRequest::create([
                'request_code' => SupplyRequest::generateRequestCode($user->department),
                'user_id' => $user->id,
    ...
    public function update(Request $request, SupplyRequest $supplyRequest)
    {
        ...
        if ($supplyRequest->user_id !== $user->id) {
            if ($request->expectsJson() || $request->ajax()) {
                return response()->json([
                    'success' => false,
                    'message' => 'Không có quyền sửa yêu cầu này'
                ], 403);
            }
        ...
        if (!in_array($supplyRequest->status, ['draft', 'rejected', 'tchc_rejected'])) {
            if ($request->expectsJson() || $request->ajax()) {
                return response()->json([
                    'success' => false,
                    'message' => 'Chỉ có thể sửa yêu cầu ở trạng thái bản nháp hoặc bị từ chối'
                ], 422);
            }
        ...
        try {
            ...
            if ($request->expectsJson() || $request->ajax()) {
                return response()->json([
                    'success' => true,
                    'message' => 'Đã cập nhật và gửi lại đơn yêu cầu thành công'
                ]);
            }
    ...
    public function destroy(SupplyRequest $request)
    {
        ...
        if ($request->user_id !== Auth::id()) {
            ...
            return redirect()->back()->with('error', 'Không có quyền xóa yêu cầu này');
        }
        
        // Only allow deleting draft and rejected requests
        if (!in_array($request->status, ['draft', 'rejected', 'tchc_rejected'])) {
            if ($request->wantsJson()) {
                return response()->json(['success' => false, 'message' => 'Chỉ có thể xóa yêu cầu ở trạng thái bản nháp hoặc bị từ chối'], 400);
            }
            return redirect()->back()->with('error', 'Chỉ có thể xóa yêu cầu ở trạng thái bản nháp hoặc bị từ chối');
        }
        ...
        try {
            ...
            return redirect()->route('dashboard.employee')->with('success', 'Đã xóa yêu cầu thành công');
            ...
            return redirect()->back()->with('error', 'Có lỗi xảy ra khi xóa yêu cầu');
        }
    }
    ...
    /**
     * Export supply requests to Excel
     * TCHC có thể export tất cả các yêu cầu từ mọi bộ phận
     */
    public function export(Request $request)
    {
        $user = Auth::user();
        
        // Kiểm tra quyền: TCHC checker/manager hoặc admin
        if (!$user->is_tchc_checker && !$user->is_tchc_manager && $user->role !== 'admin') {
            return back()->with('error', 'Bạn không có quyền export dữ liệu');
        }
        ...
        $query = SupplyRequest::with([
            'requestItems.officeSupply', 
            'user', 
            'tchcChecker', 
            'tchcManager'
        ]);

        // TCHC có thể xem tất cả bộ phận
        if ($user->is_tchc_checker || $user->is_tchc_manager || $user->role === 'admin') {
            // Không lọc theo department, trừ khi được chỉ định
            if ($department) {
                $query->where('requester_department', $department);
            }
        }

        // Lọc theo năm (chỉ khi có filter)
        if ($year) {
            $query->whereYear('created_at', $year);
        }
        ...
        // Lọc theo trạng thái (chỉ khi không phải 'all')
        ...
        $fileName = 'Danh_sach_yeu_cau_van_phong_pham';
        if ($year) {
            $fileName .= '_' . $year;
        }
        if ($period) {
            $fileName .= '_Thang_' . $period;
        }
        if ($department) {
        ...
        return Excel::download(new SupplyRequestsExport($requests), $fileName);
    }

    /**
     * Export supply items (chi tiết văn phòng phẩm) to Excel
     * TCHC có thể export chi tiết tất cả các vật phẩm từ mọi bộ phận
     */
    public function exportItems(Request $request)
    {
        ...
        // Query request items với relationships
        $query = RequestItem::with([
            'officeSupply',
            'supplyRequest.user'
        ])
        ->whereHas('supplyRequest', function($q) use ($year, $period, $department, $status, $user) {
            // TCHC có thể xem tất cả bộ phận
            if ($user->is_tchc_checker || $user->is_tchc_manager || $user->role === 'admin') {
                // Không lọc theo department, trừ khi được chỉ định
                if ($department) {
                    $q->where('requester_department', $department);
                }
            }
            ...
            // Lọc theo kỳ (tháng)
            if ($period) {
                $q->whereMonth('created_at', $period);
            }
        ...
        \Log::info('Export Items Debug', [
            ...
            'first_item' => $items->first() ? [
                'id' => $items->first()->id,
                'supply_request_id' => $items->first()->supply_request_id,
                'office_supply' => $items->first()->officeSupply ? $items->first()->officeSupply->name : 'NULL',
            ] : 'No items'
        ]);

        $fileName = 'Chi_tiet_van_phong_pham';
        if ($year) {
            $fileName .= '_' . $year;
        }
        if ($period) {
        ...
        return Excel::download(new SupplyItemsExport($items), $fileName);
    }
}
