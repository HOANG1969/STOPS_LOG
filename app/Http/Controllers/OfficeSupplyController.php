...
class OfficeSupplyController extends Controller
{
    ...
    {
        try {
            // Check if supply is being used in any requests
            if ($supply->requestItems()->exists()) {
                // If being used, just set inactive
                $supply->update(['is_active' => false]);
                $message = 'Đã ẩn văn phòng phẩm (do đã được sử dụng trong yêu cầu)';
            } else {
                // If not being used, delete permanently
                $supply->delete();
            ...
            \Log::error('Error deleting office supply', [
                'supply_id' => $supply->id,
                'error' => $e->getMessage()
            ]);
            ...
            return response()->json([
                'success' => false,
                'message' => 'Có lỗi xảy ra khi xóa văn phòng phẩm'
            ], 500);
        }
    }
