...
// Test route
include __DIR__ . '/test-import.php';
...
Route::middleware('auth')->group(function () {
    ...
    Route::get('/dashboard/approval', [App\Http\Controllers\DashboardController::class, 'approval'])->name('dashboard.approval');
    Route::post('/dashboard/approve/{supplyRequest}', [App\Http\Controllers\DashboardController::class, 'approve'])->name('dashboard.approve');
    ...
    // Admin management routes for office supplies
    Route::get('/office-supplies/admin/manage', [OfficeSupplyController::class, 'admin'])->name('office-supplies.admin.manage')->middleware('can:admin-access');
    Route::get('/office-supplies/admin/data', [OfficeSupplyController::class, 'getAllForAdmin'])->name('office-supplies.admin')->middleware('can:admin-access');
    
    // Import routes for office supplies
    Route::get('/office-supplies/import/form', [OfficeSupplyController::class, 'showImportForm'])->name('office-supplies.import.form')->middleware('can:admin-access');
    Route::post('/office-supplies/import', [OfficeSupplyController::class, 'import'])->name('office-supplies.import')->middleware('can:admin-access');
    ...
    // API Routes for Office Supplies (inside auth middleware)
    Route::get('/api/office-supplies/for-request', [OfficeSupplyController::class, 'getForRequest'])->name('office-supplies.api.for-request');

    // Supply Requests Routes
    

    Route::get('/supply-requests', [SupplyRequestController::class, 'index'])->name('supply-requests.index');
    Route::get('/stops', [App\Http\Controllers\StopController::class, 'index'])->name('stops.index');

    // Route::get('/tchc/checker/dashboard', [SupplyRequestController::class, 'index'])->name('supply-requests.index');
    Route::post('/supply-requests', [SupplyRequestController::class, 'store'])->name('supply-requests.store');
    Route::get('/supply-requests/export', [SupplyRequestController::class, 'export'])->name('supply-requests.export');
    Route::get('/supply-requests/export-items', [SupplyRequestController::class, 'exportItems'])->name('supply-requests.export-items');
    Route::get('/supply-requests/{supplyRequest}/edit', [SupplyRequestController::class, 'edit'])->name('supply-requests.edit');
    Route::put('/supply-requests/{supplyRequest}', [SupplyRequestController::class, 'update'])->name('supply-requests.update');
    Route::post('/supply-requests/{supplyRequest}/resubmit', [SupplyRequestController::class, 'resubmit'])->name('supply-requests.resubmit');
    // Route::get('/supply-requests/my-requests', [SupplyRequestController::class, 'myRequests'])->name('supply-requests.my-requests'); // Đã thay bằng supply-requests.index cho employee
    Route::get('/supply-requests/for-approval', [SupplyRequestController::class, 'forApproval'])->name('supply-requests.for-approval');
    Route::get('/supply-requests/all', [SupplyRequestController::class, 'allRequests'])->name('supply-requests.all');
    ...
    Route::delete('/supply-requests/{request}', [SupplyRequestController::class, 'destroy'])->name('supply-requests.destroy');

    // TCHC Routes
    Route::prefix('tchc')->name('tchc.')->group(function () {
        // TCHC Checker Routes  
        Route::middleware('can:tchc-checker-access')->prefix('checker')->name('checker.')->group(function () {
            Route::get('/dashboard', [App\Http\Controllers\TchcController::class, 'checkerDashboard'])->name('dashboard');
            Route::patch('/{request}/check', [App\Http\Controllers\TchcController::class, 'checkRequest'])->name('check');
        });

        // TCHC Manager Routes
        Route::middleware('can:tchc-manager-access')->prefix('manager')->name('manager.')->group(function () {
            Route::get('/dashboard', [App\Http\Controllers\TchcController::class, 'managerDashboard'])->name('dashboard');
            Route::patch('/{request}/approve', [App\Http\Controllers\TchcController::class, 'finalApprove'])->name('approve');
            Route::patch('/{request}/reject', [App\Http\Controllers\TchcController::class, 'finalReject'])->name('reject');
        });
    });

    // User Management Routes (Admin only) - Keep original structure
    Route::prefix('admin/users')->name('users.')->middleware('can:admin-access')->group(function () {
        Route::get('/', [UserManagementController::class, 'index'])->name('index');
        Route::get('/create', [UserManagementController::class, 'create'])->name('create');
        Route::post('/', [UserManagementController::class, 'store'])->name('store');
        Route::get('/import', [UserManagementController::class, 'showImportForm'])->name('import.form');
        Route::post('/import', [UserManagementController::class, 'import'])->name('import.process');
    ...
    // Admin Import Routes - Simple addition
    Route::prefix('admin/import')->name('admin.import.')->middleware('can:admin-access')->group(function () {
        Route::get('/office-supplies', [App\Http\Controllers\Admin\ImportController::class, 'showImportForm'])->name('office-supplies');
        Route::post('/office-supplies', [App\Http\Controllers\Admin\ImportController::class, 'importOfficeSupplies'])->name('office-supplies.process');
...
Route::get('/auto-login', function() {
    ...
    if ($user) {
        Auth::login($user);
        return redirect()->route('office-supplies.index')->with('success', 'Auto logged in as ' . $user->name);
    }
    return redirect()->route('login')->with('error', 'Test user not found');
})->name('auto-login');
...
// STOP (Safety Training Observation Program) Routes  
Route::middleware('auth')->group(function() {
    // Priority update route MUST be registered BEFORE resource routes
    Route::patch('stops/{stop}/priority', [App\Http\Controllers\StopController::class, 'updatePriority'])->name('stops.update-priority');
    
    // Bulk priority update route
    Route::post('stops/bulk-priority', [App\Http\Controllers\StopController::class, 'bulkUpdatePriority'])->name('stops.bulk-priority');
    
    // Resource routes for STOP CRUD
    Route::resource('stops', App\Http\Controllers\StopController::class);
    
    // STOP Reports
    Route::get('reports/stops', [App\Http\Controllers\ReportController::class, 'index'])->name('reports.index');
    Route::get('reports/stops/export-modal', [App\Http\Controllers\ReportController::class, 'exportModalReport'])->name('reports.export-modal');
});
