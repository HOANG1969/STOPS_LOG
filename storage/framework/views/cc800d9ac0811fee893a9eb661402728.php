...
<div class="container mx-auto px-6">
    <div class="bg-white rounded-lg shadow-lg p-6">
        <h1 class="text-xl font-semibold mb-6" style="font-size: 120px;">Tạo phiếu đăng ký văn phòng phẩm</h1>
    ...
    <div class="flex items-center justify-center min-h-screen p-4">
        <div class="bg-white rounded-lg shadow-xl max-w-2xl w-full">
            ...
            <div class="p-6 max-h-96 overflow-y-auto">
                ...
                <div class="supply-item flex justify-between items-center p-2 border-b hover:bg-gray-50" data-name="<?php echo e(strtolower($supply->name)); ?>">
                    <div>
                        <div class="font-medium supply-name"><?php echo e($supply->name); ?></div>
                        <div class="text-sm text-gray-500 supply-specs"><?php echo e($supply->unit); ?> - Tồn kho: <?php echo e($supply->stock_quantity); ?></div>
                    </div>
                    <button type="button" class="bg-blue-500 hover:bg-blue-600 text-white px-3 py-1 rounded text-sm select-supply"
                            data-id="<?php echo e($supply->id); ?>"
                            data-name="<?php echo e($supply->name); ?>"
                            data-unit="<?php echo e($supply->unit); ?>"
                            data-stock="<?php echo e($supply->stock_quantity); ?>">
                        Chọn
                    </button>
                </div>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
...
document.addEventListener('click', function(e) {
    if (e.target.classList.contains('select-supply')) {
        ...
        console.log('Adding item:', { id, name, unit, stock, rowIndex });

        // Tạo inputs trực tiếp trong form (không trong table)
        const form = document.getElementById('requestForm');
        const hiddenInputsContainer = document.createElement('div');
        hiddenInputsContainer.className = 'hidden-item-inputs';
        hiddenInputsContainer.dataset.itemIndex = rowIndex;
        
        hiddenInputsContainer.innerHTML = `
            <input type="hidden" name="items[${rowIndex}][supply_id]" value="${id}">
            <input type="hidden" name="items[${rowIndex}][quantity]" value="1" class="item-quantity" data-item-index="${rowIndex}">
            <input type="hidden" name="items[${rowIndex}][purpose]" value="" class="item-purpose" data-item-index="${rowIndex}">
        `;
        
        form.appendChild(hiddenInputsContainer);

        // Tạo row hiển thị trong table
        const newRow = document.createElement('tr');
        newRow.setAttribute('data-supply-id', id);
        newRow.setAttribute('data-item-index', rowIndex);
        newRow.innerHTML = `
            ...
            <td class="px-4 py-2 border">
                <input type="number" 
                       class="w-full px-2 py-1 border rounded quantity-input" 
                       min="1" max="${stock}" value="1" 
                       data-item-index="${rowIndex}"
                       onchange="updateHiddenQuantity(${rowIndex}, this.value)">
            </td>
            <td class="px-4 py-2 border">
                <input type="text" 
                       class="w-full px-2 py-1 border rounded purpose-input" 
                       placeholder="Ghi chú (tùy chọn)"
                       data-item-index="${rowIndex}"
                       onchange="updateHiddenPurpose(${rowIndex}, this.value)">
            </td>
            <td class="px-4 py-2 border">
        ...
        document.getElementById('supplyTableBody').appendChild(newRow);
        rowIndex++;
        ...
        console.log('Row added, new rowIndex:', rowIndex);
        console.log('Hidden inputs:', document.querySelectorAll('input[name*="items["]'));
        
        document.getElementById('selectSupplyModal').classList.add('hidden');
    }
});

// Cập nhật hidden inputs khi user thay đổi giá trị
function updateHiddenQuantity(index, value) {
    const hiddenInput = document.querySelector(`input[name="items[${index}][quantity]"]`);
    if (hiddenInput) {
        hiddenInput.value = value;
    }
}

function updateHiddenPurpose(index, value) {
    const hiddenInput = document.querySelector(`input[name="items[${index}][purpose]"]`);
    if (hiddenInput) {
        hiddenInput.value = value;
    }
}
...
function removeItem(index) {
    ...
    // Xóa hidden inputs
    const hiddenContainer = document.querySelector(`div[data-item-index="${index}"]`);
    if (hiddenContainer) {
    ...
    const remainingRows = tableBody.querySelectorAll('tr:not(#emptyTableMessage)');
    const emptyMessage = document.getElementById('emptyTableMessage');
...
// Remove row function - cập nhật để hoạt động với approach mới
...
// Form validation - đơn giản hóa và kiểm tra hidden inputs
document.getElementById('requestForm').addEventListener('submit', function(e) {
    const rows = document.querySelectorAll('#supplyTableBody tr:not(#emptyTableMessage)');
    const hiddenInputs = document.querySelectorAll('input[name*="items["][name$="][supply_id]"]');
    
    console.log('Form submit - rows:', rows.length, 'hidden inputs:', hiddenInputs.length);
    
    if (rows.length === 0 || hiddenInputs.length === 0) {
        e.preventDefault();
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\KDNVPP-new\KDNVPP-new\resources\views\employee\requests\create-simple.blade.php ENDPATH**/ ?>