...
class TestDataSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        // Tạo supply request với status tchc_approved để test view
        $tchcManager = User::where('is_tchc_manager', true)->first();
        $request3 = SupplyRequest::create([
            'request_code' => 'REQ' . date('YmdHis') . '003',
            'user_id' => $employee->id,
            'requester_name' => $employee->name,
            'requester_email' => $employee->email,
            'requester_position' => $employee->position,
            ...
            'tchc_checker_id' => $tchcChecker ? $tchcChecker->id : null,
            'tchc_checked_at' => now()->subHour(),
            'tchc_check_notes' => 'Đã kiểm tra, hợp lệ',
            'tchc_manager_id' => $tchcManager ? $tchcManager->id : null,
            'tchc_approved_at' => now(),
            'tchc_approval_notes' => 'Phê duyệt cuối hoàn tất',
            'notes' => 'Test request - đã hoàn thành workflow'
        ]);

        // Tạo request items cho request3
        RequestItem::create([
            'supply_request_id' => $request3->id,
            'office_supply_id' => $supply->id,
            'quantity' => 20,
            'purpose' => 'Dùng cho dự án khẩn cấp'
        ]);
